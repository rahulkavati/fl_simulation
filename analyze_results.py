...
Analyze the FHE CKKS data flow results and display key metrics
"""
...
def compare_with_plaintext():
    ...
    print("   ")
    print("   fl_config = FLConfig(rounds=2, clients=4)")
...
def create_visualization_script():
    """
    Create a script for visualizing results
    """
...
def plot_performance_metrics(results_file):
    """Plot performance metrics from results"""
    
    with open(results_file, 'r') as f:
        results = json.load(f)
    
    detailed_results = results['detailed_results']
    rounds = [r['round'] for r in detailed_results]
    
    # Extract timing data
    total_times = [r['timing']['total_time'] for r in detailed_results]
    encryption_times = [r['timing']['avg_router_encryption'] for r in detailed_results]
    aggregation_times = [r['timing']['server_aggregation'] for r in detailed_results]
    decryption_times = [r['timing']['avg_router_decryption'] for r in detailed_results]
    
    # Create plots
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(10, 8))
    
    # Plot 1: Total time per round
    ax1.plot(rounds, total_times, 'b-o', label='Total Time')
    ax1.set_xlabel('Round')
    ax1.set_ylabel('Time (seconds)')
    ax1.set_title('FHE CKKS Federated Learning - Total Time per Round')
    ax1.grid(True)
    ax1.legend()
    
    # Plot 2: FHE operations breakdown
    ax2.plot(rounds, encryption_times, 'r-o', label='Encryption')
    ax2.plot(rounds, aggregation_times, 'g-o', label='Aggregation')
    ax2.plot(rounds, decryption_times, 'b-o', label='Decryption')
    ax2.set_xlabel('Round')
    ax2.set_ylabel('Time (seconds)')
    ax2.set_title('FHE Operations Breakdown')
    ax2.grid(True)
    ax2.legend()
    
    plt.tight_layout()
    plt.savefig('fhe_performance_analysis.png', dpi=300, bbox_inches='tight')
    plt.show()
    
    print("📊 Performance plots saved as 'fhe_performance_analysis.png'")

# Usage
plot_performance_metrics('results/fhe_data_flow_results_TIMESTAMP.json')
'''
...
if __name__ == "__main__":
    ...
    if results:
        ...
        print("\n❌ ANALYSIS FAILED!")
        print("Please run the data flow first: python run_fhe_data_flow.py")
